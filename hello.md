...
**FilesystemMiddleware** provides seven tools to help your agent interact with both short-term and long-term memory:
- **ls**: List files in a directory
- **read_file**: Read a file, optionally with `offset` and `limit`
- **preview_file**: Quickly read the first N lines of a file
- **write_file**: Write a new file to your filesystem
- **edit_file**: Perform exact string replacements in files
- **glob**: Find files matching a glob pattern (e.g., `**/*.py`)
- **grep**: Search for text across files with optional glob filtering
...
agent = create_agent(
    ...
    middleware=[
        FilesystemMiddleware(
            backend=...,  # Optional: customize storage backend (StateBackend by default)
            system_prompt="Write to the filesystem when...",  # Optional: custom system prompt override
            custom_tool_descriptions={
                "ls": "Use ls to explore directories first",
                "read_file": "Use read_file for full reads or with offset/limit",
                "preview_file": "Use preview_file to sanity-check file contents",
                "edit_file": "Use edit_file only after reading the file",
                "glob": "Use glob to find files by pattern",
                "grep": "Use grep to search text across files"
            },
        ),
    ],
)
...
Prior versions of deepagents separated sync and async agent factories. `async_create_deep_agent` has been folded into `create_deep_agent`.

**Use `create_deep_agent` for both sync and async agents.** It returns a runnable that supports both blocking `invoke` and async streaming via `astream`.
...
**NOTE:** MCP tools are async-compatible. Use `create_deep_agent` and stream results with `astream` as shown below.
